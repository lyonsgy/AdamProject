...
- (void)was_setCircleImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage fillColor:(UIColor *)color{
    [self was_setCircleImageWithUrlString:urlString placeholder:placeholderImage fillColor:color opaque:color];
}

//网络延迟下载--圆形 背景色为透明 无背景色
- (void)was_setCircleImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage{
    [self was_setCircleImageWithUrlString:urlString placeholder:placeholderImage fillColor:nil opaque:NO];
}
...
- (void)was_setRoundRectImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage fillColor:(UIColor *)color cornerRadius:(CGFloat) cornerRadius{
    [self was_setRoundRectImageWithUrlString:urlString placeholder:placeholderImage fillColor:color opaque:color cornerRadius:cornerRadius];
}
...
- (void)was_setRoundRectImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage cornerRadius:(CGFloat) cornerRadius{
    [self was_setRoundRectImageWithUrlString:urlString placeholder:placeholderImage fillColor:nil opaque:NO cornerRadius:cornerRadius];
}

- (void)was_setCircleImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage fillColor:(UIColor *)color opaque:(BOOL)opaque{
    ...
    if (placeholderImage) {
        ...
        [placeholderImage was_roundImageWithSize:size fillColor:color opaque:opaque completion:^(UIImage *radiusPlaceHolder) {
            //2.使用sd的方法缓存异步下载的图片
            [weakSelf sd_setCircleImageWithURL:url placeholderImage:radiusPlaceHolder progressViewType:NPProgressViewTypeCircleSmall completed:^(BOOL isFinished, UIImage *image, SDImageCacheType cacheType) {
                if (isFinished) {
...
- (void)was_setRoundRectImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage fillColor:(UIColor *)color opaque:(BOOL)opaque cornerRadius:(CGFloat) cornerRadius{
    [self.superview layoutIfNeeded];
    NSURL *url = [NSURL URLWithString:urlString];
    //防止循环引用
    __weak typeof(self) weakSelf = self;
    ...
    if (placeholderImage) {
        //占位图片不为空的情况
        //1.现将占位图圆角化，这样就避免了如图片下载失败，使用占位图的时候占位图不是圆角的问题
        [placeholderImage was_roundRectImageWithSize:size fillColor:color opaque:opaque radius:cornerRadius completion:^(UIImage *roundRectPlaceHolder) {
            //2.使用sd的方法缓存异步下载的图片
            [weakSelf sd_setCircleImageWithURL:url placeholderImage:roundRectPlaceHolder progressViewType:NPProgressViewTypeCircleSmall completed:^(BOOL isFinished, UIImage *image, SDImageCacheType cacheType) {
                if (isFinished) {
                    [image was_roundRectImageWithSize:image.size fillColor:color opaque:opaque radius:((image.size.height/size.height)*cornerRadius) completion:^(UIImage *radiusImage) {
                        weakSelf.image = radiusImage;
                    }];
                }
            } animated:true];
...
- (void)was_setCircleImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage fillColor:(UIColor *)color forState:(UIControlState)state {
    [self was_setCircleImageWithUrlString:urlString placeholder:placeholderImage fillColor:color opaque:color forState:state];
}
...
- (void)was_setCircleImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage forState:(UIControlState)state{
    [self was_setCircleImageWithUrlString:urlString placeholder:placeholderImage fillColor:nil opaque:NO forState:state];
}
...
- (void)was_setRoundRectImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage fillColor:(UIColor *)color cornerRadius:(CGFloat) cornerRadius forState:(UIControlState)state{
    ...
    [self was_setRoundRectImageWithUrlString:urlString placeholder:placeholderImage fillColor:color opaque:color cornerRadius:cornerRadius forState:state];
}
...
- (void)was_setRoundRectImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage cornerRadius:(CGFloat) cornerRadius forState:(UIControlState)state{
    [self was_setRoundRectImageWithUrlString:urlString placeholder:placeholderImage fillColor:nil opaque:NO cornerRadius:cornerRadius forState:state];
}
...
- (void)np_setImageWithURL:(NSURL *)url forState:(UIControlState)state placeholderImage:(UIImage *)placeholder radius:(CGFloat)radius fillColor:(UIColor *)fillColor{
    {
        ...
        [self sd_setImageWithURL:url forState:state placeholderImage:placeholder completed:^(UIImage *img, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            //3.如果下载成功那么讲下载成功的图进行圆角化
            if (error) {
                [placeholder was_roundRectImageWithSize:wself.frame.size fillColor:fillColor opaque:YES radius:radius completion:^(UIImage *roundRectImage) {
                    [wself setImage:roundRectImage forState:state];
                }];
            }else{
                [img was_roundRectImageWithSize:wself.frame.size fillColor:fillColor opaque:YES radius:radius completion:^(UIImage *roundRectImage) {
...
- (void)was_setRoundRectImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage fillColor:(UIColor *)color opaque:(BOOL)opaque cornerRadius:(CGFloat) cornerRadius forState:(UIControlState)state{
    NSURL *url = [NSURL URLWithString:urlString];
    [self.superview layoutIfNeeded];
    __weak typeof(self) weakSelf = self;
    CGSize size = self.frame.size;
...
- (void)was_setCircleImageWithUrlString:(NSString *)urlString placeholder:(UIImage *)placeholderImage fillColor:(UIColor *)color opaque:(BOOL)opaque forState:(UIControlState)state{
    NSURL *url = [NSURL URLWithString:urlString];
    [self.superview layoutIfNeeded];
    __weak typeof(self) weakSelf = self;
    CGSize size = self.frame.size;
