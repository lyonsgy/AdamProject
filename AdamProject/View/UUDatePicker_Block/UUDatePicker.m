...
{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point_x+(self.frame.size.width-320)/2, (self.frame.size.height-20)/2, 20, 20)];
    label.text = name;
...
- (void)playTheDelegate
{
    ...
    [self.delegate uuDatePicker:self
                        ...
                        weekDay:strWeekDay];
    NSString *string=[NSString stringWithFormat:@"%@-%@-%@ %@:%@",yearArray[yearIndex],monthArray[monthIndex],dayArray[dayIndex],hourArray[hourIndex],minuteArray[minuteIndex]];
    NSDateFormatter *formatter=[[NSDateFormatter alloc]init];
    [formatter setDateFormat:@"YYYY-MM-dd HH:mm"];
    _currentDate=[formatter dateFromString:string];
}
